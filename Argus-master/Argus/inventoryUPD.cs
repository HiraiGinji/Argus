        ...
        public void searchInfo()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        dateTimePicker1.Value = Convert.ToDateTime(row["EXPIRY_DATE"]);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void updateFunction(string pid)
        {
            try
            {
                ...
                else
                {
                    ...
                    string expiry = selectedDate.ToString() ;

                    connect.updateRecord(itmName, itmPrice, itmQuantity,selectedDate, pid);
                    displayInventory(new MySqlCommand("SELECT * FROM INVENTORY"));
                }
            }
            catch (Exception ex)
