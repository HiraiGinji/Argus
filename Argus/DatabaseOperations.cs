...
namespace Argus
{
    public class DatabaseOperations
    {
        ...
        public void addRecord(string name, string price, string quantity, string category, string brand)
        {
            try
            {
                OpenConnection();
                MySqlCommand query = new MySqlCommand($"SELECT COUNT(*) FROM INVENTORY WHERE PNAME = '{name}' AND BRAND = '{brand}' AND " +
                    $"CATEGORY = '{category}'", connection);
                int exists = Convert.ToInt32(query.ExecuteScalar());

                if (exists == 1)
                {
                    MessageBox.Show("Record currently exists.");
                }
                else if (exists == 0)
                {
                    string randomID = GenerateRandomID(8);

                    MySqlCommand cmd = new MySqlCommand($"INSERT INTO INVENTORY VALUES ('{randomID}','{brand}','{category}'," +
        ...
        public void updateRecord(string name, string price, string quantity, string category, string brand, string pid)
        {
            try
            {
                OpenConnection();

                MySqlCommand query = new MySqlCommand($"SELECT COUNT(*) FROM INVENTORY WHERE PRODUCTID = '{pid}'", connection);
                int exists = Convert.ToInt32(query.ExecuteScalar());
                ...
                {
                    MySqlCommand cmd = new MySqlCommand($"UPDATE INVENTORY SET PNAME = '{name}', ITEMPRICE = {price}, QUANTITY = {quantity}," +
                        $" CATEGORY = '{category}', BRAND = '{brand}' WHERE PRODUCTID = '{pid}'", connection);
                    cmd.ExecuteNonQuery();
        ...
        public void deleteRecord(string pid)
        {
            try
            {
                ...
                {
                    MySqlCommand cmd = new MySqlCommand($"DELETE FROM INVENTORY WHERE PRODUCTID = '{pid}'", connection);
                    cmd.ExecuteNonQuery();
        ...
        public string GetProductId(string pname, string category, string brand)
        {
            string productId = "";
            try
            {
                string query = "SELECT PRODUCTID FROM INVENTORY WHERE PNAME = @PNAME AND CATEGORY = @CATEGORY AND BRAND = @BRAND LIMIT 1";
